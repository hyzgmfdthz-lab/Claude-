...
async function ladeMannschaft(a, container) {
  try {
    ...
    /*
     * FIX (Nutzermeldung 23.09.2026, Screenshot mit dem Wort "false" direkt
     * über der Tabelle): `container.replaceChildren(...)` ist die NATIVE
     * DOM-Methode, nicht der hauseigene h()/append()-Helfer - sie filtert
     * anders als dieser keine falsy-Werte heraus, sondern wandelt jedes
     * Nicht-Node-Argument per String() in einen Textknoten um. War
     * `frei.length === 0` (keine leeren Leiharbeiterplätze mehr - hier
     * war das vorher IMMER der Fall gewesen, deshalb ist der Fehler erst
     * jetzt aufgefallen), stand hier buchstäblich der Text "false" über
     * der Tabelle. Jetzt werden falsy Eintraege vorher herausgefiltert.
     */
