...
import {
  ...
  seedDataset, defaultRoutingTemplates, defaultWorkplaces, defaultConfig, splitOrbital,
...
export function createApi(store, options = {}) {
  ...
  function vorschlagKonvergent(quelle) {
    ...
    /*
     * Terminierung einmal rechnen, Verspaetung und Reibung daraus ablesen -
     * keine doppelte Simulation.
     *
     * FIX (Nutzerpruefung 26.09.2026): zwei Fehlversuche zeigten, warum
     * beide Teile hier zusammengehoeren.
     *
     * 1. "Reibung" zaehlte zunaechst nur "Schicht ohne Arbeit" - eine zweite,
     * ebenso ernste Form blieb unbeachtet: "Budget der Qualifizierten
     * ausgeschoepft" (`plan.unassignedHours`). Werden mehrere Arbeitsgaenge
     * GLEICHZEITIG hochgesetzt, kann die gemeinsam benoetigte, aber geteilte
     * Mannschaft an einem Tag fuer den EINEN Arbeitsgang ausreichen und fuer
     * den ANDEREN schon aufgebraucht sein - nachgewiesen an echten Daten:
     * 269,9 Stunden ohne jede Person, obwohl die Terminierung sie fuer
     * ausfuehrbar hielt.
     *
     * 2. Der erste Versuch, das zu beheben, rechnete hier mit
     * `assignPeople(result, cfg, {})` - OHNE Vorzug/Notloesung (`extra`
     * leer), aus Geschwindigkeitsgruenden. Das Ergebnis: die Auswahl traf
     * Entscheidungen nach einem Bild des Personaleinsatzes, das mit dem
     * ECHTEN Einsatzplan (der Vorzug/Notloesung nutzt) nicht uebereinstimmt
     * - am echten Einsatzplan gemessen wurde es dadurch NICHT besser,
     * sondern schlechter (462 statt 269,9 Stunden offen), weil Vorzug im
     * echten Plan woanders schon Abhilfe schafft, die hier unsichtbar
     * blieb. Deshalb bekommt `assignPeople` hier dieselben `extra`
     * (`projects`/`templates`) wie der ECHTE Einsatzplan (siehe
     * `schichtvorschlag`/`assignment`/`belegung` unten) - die Bewertung
     * sieht jetzt genau das, was am Ende tatsaechlich zaehlt.
     *
     * 3. Ein DRITTER Fehlversuch (noch vor dem obigen) rechnete beide Formen
     * zu EINER Zahl zusammen (`schichtOhneArbeit + unassignedHours / 7,5`)
     * und liess Nachschliff/Ergaenzung nach dieser Summe entscheiden. Das
     * erlaubt einen schlechten Handel: eine einzelne "Schicht ohne Arbeit"
     * gegen viele Stunden "Budget der Qualifizierten ausgeschoepft"
     * eintauschen, weil die Summe trotzdem sinkt - am echten Einsatzplan
     * gemessen (mit den echten `extra` von oben!) wurden so aus 269,9
     * Stunden 491,2 Stunden, waehrend "Schicht ohne Arbeit" brav auf 0 fiel.
     * Beide Formen sind derselbe FEHLER, aber keine WAEHRUNG, in der man die
     * eine gegen die andere eintauschen darf. Deshalb entscheiden
     * Nachschliff/Ergaenzung unten ueber PARETO: eine Aenderung gilt nur
     * dann als Verbesserung, wenn KEINE der beiden Formen zunimmt und
     * WENIGSTENS eine davon abnimmt (siehe `paretoBesser` unten) - nie auf
     * Kosten der jeweils anderen.
     */
    const bewerten = (cfg) => {
      const result = planeDurch({ ...basis, config: cfg });
      const verspaetung = verspaetungImFenster({ ...basis, config: cfg }, result);
      const plan = assignPeople(result, cfg, {}, { projects: basis.projects, templates: basis.templates });
      let letzterArbeitstag = null;
      for (const t of plan.days) if ((t.entries?.length ?? 0) > 0) letzterArbeitstag = t.date;
      let schichtOhneArbeit = 0;
      for (const t of plan.days) {
        if (letzterArbeitstag && t.date > letzterArbeitstag) continue;
        for (const i of t.idle ?? []) if (i.schicht > 1 && i.grund === 'SCHICHT_OHNE_ARBEIT') schichtOhneArbeit++;
      }
      const unassignedHours = round2(plan.unassignedHours ?? 0);
      return { verspaetung, schichtOhneArbeit, unassignedHours, plan, letzterArbeitstag };
    };
    /**
     * `a` ist gegenueber `b` eine Verbesserung, wenn keine der beiden
     * Reibungsformen zunimmt und wenigstens eine davon abnimmt - nie ein
     * Eintausch der einen gegen die andere (siehe Kommentar an `bewerten`).
     */
    const paretoBesser = (a, b) => a.schichtOhneArbeit <= b.schichtOhneArbeit
      && a.unassignedHours <= b.unassignedHours + 0.01
      && (a.schichtOhneArbeit < b.schichtOhneArbeit || a.unassignedHours < b.unassignedHours - 0.01);
    const NACHSCHLIFF_TOLERANZ_TAGE = 2;

    /*
     * Aeussere Konvergenz (Nutzerpruefung 26.09.2026): Konvergenz (unten),
     * Nachschliff und Ergaenzung koennen sich GEGENSEITIG neue
     * Moeglichkeiten eroeffnen - eine hochgesetzte Schicht macht eine
     * andere manchmal erst noetig (mehr Auslastung dort) oder erst
     * ueberfluessig (die Person hat jetzt woanders Arbeit). Ein einzelner
     * Durchlauf reicht deshalb nicht immer: durch einen echten Test
     * entdeckt, nicht vermutet - "laeuft es schon so, wird nichts
     * uebernommen" fand nach dem tatsaechlichen Uebernehmen noch drei
     * weitere, echte Verbesserungen (ORBITAL_KEHLNAHT, HYDRO, ENDKONTROLLE).
     * Deshalb wird die GESAMTE Kette wiederholt, bis ein voller Durchlauf
     * nichts mehr aendert - erst dann ist der Vorschlag wirklich zu Ende
     * gerechnet, und ein zweiter Aufruf auf dem Ergebnis findet nichts mehr.
     */
    const AUSSEN_KAPPE = 5;
    for (let aussenRunde = 0; aussenRunde < AUSSEN_KAPPE; aussenRunde++) {
      let aussenGeaendert = false;

      for (let runde = 0; runde < 5; runde++) {
        const input = { ...basis, config };
        const result = (aussenRunde === 0 && runde === 0) ? resultVorher : planeDurch(input);
      ...
      /*
       * Nachschliff: Schichten wieder herausnehmen, die im ENDSTAND kaum
       * noch etwas retten, aber Leute ohne passende Arbeit in eine ganze
       * Zusatzschicht zwingen ("Schicht ohne Arbeit").
       *
       * `planeSchichten` sieht das nicht - es rechnet nur Termine, nicht
       * Personen (dafuer muesste es `assignPeople` je Kandidat aufrufen,
       * das waere in der Kandidatensuche selbst zu teuer und, wie die
       * Nutzerpruefung 25.09.2026 zeigte, auch gefaehrlich: eine hoehere
       * Huerde MITTEN in der Kandidatensuche aendert die Reihenfolge, in
       * der Kandidaten getestet werden, und damit den ganzen weiteren
       * Verlauf - in der Probe wurde dadurch das Ergebnis nicht schlanker,
       * sondern gleichzeitig schlechter UND unruhiger (311 -> 331
       * Verspaetungstage, 17 -> 25 mal Schicht ohne Arbeit).
       *
       ...
       * Kostet das Herausnehmen nur wenig Termintreue (hoechstens 2 Tage)
       * und baut es tatsaechlich "Schicht ohne Arbeit" ab, bleibt sie
       * draussen - sonst bleibt sie drin. Das aendert nichts an der Suche
       * selbst, nur an der fertigen Auswahl.
       */
      if (Object.keys(aenderungenJeOp).length > 0) {
        let beste = bewerten(config);
        /*
         * Ein einzelner Durchlauf reicht nicht immer: manche Arbeitsgaenge
         * sind erst GEMEINSAM ueberfluessig - einzeln fuer sich, waehrend
         * der jeweils andere noch drinsteht, kostet die Ruecknahme noch
         * mehr als die Toleranz erlaubt. Deshalb wird wiederholt, bis eine
         * Runde nichts mehr findet.
         */
        for (let nachschliffRunde = 0; nachschliffRunde < 5; nachschliffRunde++) {
          ...
          for (const opId of Object.keys(aenderungenJeOp)) {
            ...
            const ohne = bewerten(configOhne);
            if (ohne.verspaetung - beste.verspaetung <= NACHSCHLIFF_TOLERANZ_TAGE && paretoBesser(ohne, beste)) {
       ...
       * ohne Arbeit" - z. B. weil "keiner darf alleine arbeiten" mehr
       * Leute in eine Schicht zwingt, als es dort qualifizierte gibt),
       * bekommt wenn moeglich einen EIGENEN Arbeitsgang fuer GENAU DIESE
       * EINE WOCHE einen Grad hoeher gesetzt - wenn dafuer wirklich zu ihm
       * passende Arbeit wartet UND das die Verspaetung nicht
       * verschlechtert. Nutzeranfrage 25.09.2026: "es sollte die beste
       * Option genutzt werden" (genannt: Hydroprüfung, Entgraten, ein
       * dritter Heftplatz, ein zweiter Sägeplatz - alles Arbeitsgänge, an
       * denen die Person bereits qualifiziert waere, wenn die Schicht dort
       * liefe).
       *
       ...
       * Option uebernommen (die meiste "Schicht ohne Arbeit" abgebaut, bei
       * Gleichstand das wenigste zusaetzliche `unassignedHours`, siehe
       * `paretoBesser` an `bewerten` oben) - dann neu bewertet, bis nichts
       * mehr hilft oder die Rundenobergrenze erreicht ist.
       */
      for (let ergaenzungRunde = 0; ergaenzungRunde < 5; ergaenzungRunde++) {
        const beste = bewerten(config);
        const { plan: bestePlan, letzterArbeitstag } = beste;
        if (beste.schichtOhneArbeit === 0) break;
        ...
        for (const { wk, schicht, ops } of betroffen.values()) {
          ...
          for (const opId of ops) {
            ...
            const probeBewertung = bewerten(configProbe);
            if (probeBewertung.verspaetung > beste.verspaetung) continue; // darf Termine nicht verschlechtern
            if (!paretoBesser(probeBewertung, beste)) continue; // muss wirklich etwas bringen, ohne die andere Reibung zu verschlechtern
            if (!bestKandidat || probeBewertung.schichtOhneArbeit < bestKandidat.bewertung.schichtOhneArbeit
              || (probeBewertung.schichtOhneArbeit === bestKandidat.bewertung.schichtOhneArbeit
                && probeBewertung.unassignedHours < bestKandidat.bewertung.unassignedHours)) {
              bestKandidat = { opId, eintrag: probe[opId], config: configProbe, bewertung: probeBewertung };
            }
          }
        }
  ...
  const api = {
    ...
    importProjects({ format, data, mode = 'merge' }) {
      ...
      if (format === 'xlsx') {
        ...
        /*
         * FIX (Nutzermeldung 24.09.2026, "die Datensicherung habe ich hier
         * hochgeladen"): Eine JSON-Datensicherung wurde ueber dieses Feld
         * (gedacht fuer Excel/CSV-Projektlisten) als CSV eingelesen. readCsv
         * ist absichtlich tolerant - aus jeder Zeile der JSON-Datei wurde
         * dadurch eine "Zeile", und importRows() fand irgendwo darin
         * zufaellig ein Wort wie "Projekt" oder "Auftrag" als vermeintliche
         * Kopfzeile. Ergebnis: mehrere tausend Muell-Projekte, im Modus
         * "ersetzen" sogar anstelle der echten Auftraege. Eine JSON-Datei
         * wird deshalb jetzt VOR dem Parsen erkannt und klar abgelehnt,
         * statt sie als Tabelle misszuverstehen.
         */
        if (/^\s*[{[]/.test(String(data))) {
          throw new ApiError('Diese Datei sieht nach einer Datensicherung (JSON) aus, nicht nach einer '
            + 'Projektliste (Excel/CSV). Dieses Feld ist nur für Auftragslisten gedacht.');
        }
...
export function importRows(dataset, rows, mode = 'merge') {
  /*
   * FIX (Nutzermeldung 24.09.2026): eine echte Kopfzeile steht immer ganz
   * oben. Ohne diese Grenze fand die Suche bei einer falsch hochgeladenen,
   * sehr grossen Datei irgendwo mitten drin zufaellig eine Zeile mit einem
   * Treffer und tat danach so, als waere alles Weitere eine Projektliste -
   * mit tausenden Muell-"Projekten" als Folge.
   */
  const headerIdx = rows.slice(0, 20).findIndex((r) => r && r.some((c) => /auftrag|projekt|kunde/i.test(String(c ?? ''))));
  if (headerIdx < 0) throw new ApiError('Es wurde keine Kopfzeile mit "Auftrag" oder "Projekt" in den ersten 20 Zeilen gefunden.');
...
/**
 * FIX (Nutzermeldung 23.09.2026, Termintreue sprang unerklärt auf 95 %):
 * `dataset.templates ??= defaultRoutingTemplates()` greift nur, wenn noch
 * GAR KEINE Arbeitsfolgen gespeichert sind - bei einem echten, lange
 * genutzten Datenbestand sind sie es laengst, und bleiben dann fuer immer
 * auf dem Stand von damals stehen. Jede Aenderung an der Arbeitsfolge in
 * diesem Projekt (Arbeitsvorbereitung ergaenzt, Orbitalschweissen in
 * Kehlnaht/Stumpfnaht aufgeteilt, Handschweissen/Molchen ergaenzt) erreicht
 * dadurch NIE die echten, gespeicherten Arbeitsplaene - nur druckfrische
 * Datenbestaende. Belegt am echten Datenexport des Nutzers: ein "Neubau 40
 * ft"-Auftrag zeigte 147,6 h Gesamtstunden statt der dokumentierten 247,75 h
 * - fehlend war genau der Anteil, den Orbitalschweissen (~90 h) und
 * Arbeitsvorbereitung ausmachen. Die Rechnung war dadurch nicht falsch,
 * sondern rechnete mit einem Arbeitsplan, der einen ganzen, oft den
 * groessten Arbeitsgang gar nicht mehr kannte - daher die unrealistisch
 * gute Termintreue.
 *
 * Diese Funktion ergaenzt bestehende, gespeicherte Arbeitsplaene um neu
 * hinzugekommene Arbeitsgaenge, OHNE von der Abteilung selbst geaenderte
 * Stunden an den ohnehin vorhandenen Schritten anzutasten.
 */
function migrateArbeitsplaene(dataset) {
  const frisch = defaultRoutingTemplates();
  for (const [key, neu] of Object.entries(frisch)) {
    const alt = dataset.templates[key];
    if (!alt || !Array.isArray(alt.steps)) continue;

    /*
     * 1) Das alte, ungeteilte "ORBITAL" (vor 23.09.2026) in Kehlnaht und
     *    Stumpfnaht aufteilen - mit den TATSAECHLICH hinterlegten Stunden,
     *    nicht dem Standardwert, falls die Abteilung sie selbst geaendert
     *    hatte. splitOrbital() haelt die Summe exakt gleich.
     */
    const orbitalIndex = alt.steps.findIndex((s) => s.opId === 'ORBITAL');
    const hatKehlnahtSchon = alt.steps.some((s) => s.opId === 'ORBITAL_KEHLNAHT');
    if (orbitalIndex !== -1 && !hatKehlnahtSchon) {
      const altSchritt = alt.steps[orbitalIndex];
      const aufgeteilt = splitOrbital(Number(altSchritt.hours) || 0);
      const kehlnahtVorlage = neu.steps.find((s) => s.opId === 'ORBITAL_KEHLNAHT');
      const stumpfnahtVorlage = neu.steps.find((s) => s.opId === 'ORBITAL_STUMPFNAHT');
      const ersatz = [];
      if (kehlnahtVorlage) ersatz.push({ ...kehlnahtVorlage, hours: aufgeteilt.ORBITAL_KEHLNAHT });
      if (stumpfnahtVorlage) ersatz.push({ ...stumpfnahtVorlage, hours: aufgeteilt.ORBITAL_STUMPFNAHT });
      alt.steps.splice(orbitalIndex, 1, ...ersatz);
      /*
       * Jede andere Position, die noch auf das alte "ORBITAL" als Vorgaenger
       * zeigt (z. B. Beizen), muss jetzt auf BEIDE neuen Positionen zeigen -
       * sonst wartet sie auf einen Arbeitsgang, den es nicht mehr gibt.
       */
      for (const schritt of alt.steps) {
        if (!Array.isArray(schritt.predecessors)) continue;
        const idx = schritt.predecessors.findIndex((p) => p.opId === 'ORBITAL');
        if (idx === -1) continue;
        const alterEintrag = schritt.predecessors[idx];
        schritt.predecessors.splice(idx, 1,
          { ...alterEintrag, opId: 'ORBITAL_KEHLNAHT' },
          { ...alterEintrag, opId: 'ORBITAL_STUMPFNAHT' });
      }
    }

    /*
     * 2) Fehlende neue Arbeitsgaenge (Arbeitsvorbereitung, Handschweissen,
     *    Molchen) aus der aktuellen Standardvorlage ergaenzen - mit ihren
     *    Standardstunden (Arbeitsvorbereitung hat reale Stunden,
     *    Handschweissen/Molchen standardmaessig 0 h je Nutzervorgabe).
     */
    const vorhanden = new Set(alt.steps.map((s) => s.opId));
    for (const schritt of neu.steps) {
      if (vorhanden.has(schritt.opId)) continue;
      // Direkt nach dem Arbeitsgang einfuegen, der laut aktueller Vorlage
      // davor liegt - existiert der nicht (mehr) im alten Plan, ans Ende.
      const vorgaengerOpId = schritt.predecessors?.[0]?.opId;
      const vorgaengerIndex = alt.steps.findIndex((s) => s.opId === vorgaengerOpId);
      const position = vorgaengerIndex !== -1 ? vorgaengerIndex + 1 : alt.steps.length;
      const eingefuegt = { ...schritt };
      alt.steps.splice(position, 0, eingefuegt);
      vorhanden.add(schritt.opId);

      /*
       * Wer bisher OHNE Vorgaenger begann (der alte erste Schritt), bekommt
       * den neu eingefuegten Arbeitsgang als Vorgaenger, wenn genau das
       * laut aktueller Vorlage jetzt so vorgesehen ist (z. B. Arbeits-
       * vorbereitung vor Saegen). Ohne das wuerde der neue Arbeitsgang zwar
       * mitgezaehlt, aber nichts mehr auf ihn warten lassen.
       */
      for (const anderer of alt.steps) {
        if (anderer === eingefuegt || anderer.predecessors?.length) continue;
        const sollVorgaenger = neu.steps.find((n) => n.opId === anderer.opId)
          ?.predecessors?.some((p) => p.opId === eingefuegt.opId);
        if (sollVorgaenger) anderer.predecessors = [{ opId: eingefuegt.opId, type: 'FS', leadHours: null }];
      }
    }
  }
}
...
/**
 * Zweite Haelfte derselben Ursache wie migrateArbeitsplaene(): nicht nur
 * die VORLAGE, auch einzelne AUFTRAEGE koennen einen eigenen Eintrag fuer
 * das alte, ungeteilte "ORBITAL" hinterlegt haben (z. B. eine Fertigmeldung
 * oder projektspezifische Reststunden). Belegt durch die Datenpruefung des
 * Nutzers selbst: mehrere Auftraege meldeten den Befund
 * ARBEITSGANG_UNBEKANNT("ORBITAL") - der Eintrag existierte, wurde aber
 * von der Terminierung stillschweigend uebergangen, weil "ORBITAL" keine
 * gueltige Arbeitsgang-Kennung mehr ist.
 */
function migrateProjektArbeitsgaenge(dataset) {
  for (const p of dataset.projects ?? []) {
    if (!Array.isArray(p.operations)) continue;
    const idx = p.operations.findIndex((o) => o.opId === 'ORBITAL');
    if (idx === -1 || p.operations.some((o) => o.opId === 'ORBITAL_KEHLNAHT')) continue;
    const alt = p.operations[idx];
    const kehlnaht = { ...alt, opId: 'ORBITAL_KEHLNAHT' };
    const stumpfnaht = { ...alt, opId: 'ORBITAL_STUMPFNAHT' };
    for (const feld of ['plannedHours', 'remainingHours']) {
      const wert = Number(alt[feld]);
      if (alt[feld] == null || alt[feld] === '' || !Number.isFinite(wert)) continue;
      const aufgeteilt = splitOrbital(wert);
      kehlnaht[feld] = aufgeteilt.ORBITAL_KEHLNAHT;
      stumpfnaht[feld] = aufgeteilt.ORBITAL_STUMPFNAHT;
    }
    p.operations.splice(idx, 1, kehlnaht, stumpfnaht);
  }
}
...
function migrate(dataset) {
  ...
  dataset.projects ??= [];
  dataset.templates ??= defaultRoutingTemplates();
  migrateArbeitsplaene(dataset);
  migrateProjektArbeitsgaenge(dataset);
